...
export function ListParagraph({ content, blogId, elementId, onContentUpdated, onElementDeleted, hyperlink }: ElementComponentProps) {
  ...
  const initial = (content ?? { title: '', text_before_list: '', list_items: [], text_after_list: '' }) as ListParagraphContent
  const { draft, patch, reset, commit, rebase, isDirty } = useElementDraft<ListParagraphContent>(initial)

  useEffect(() => { rebase((content ?? { title: '', text_before_list: '', list_items: [], text_after_list: '' }) as ListParagraphContent) }, [content])

  const saveFn = useCallback(async (data: ListParagraphContent) => {
  ...
  const view = (content ?? {}) as ListParagraphContent
  ...
  return (
    <BaseElement content={content} blogId={blogId} elementId={elementId} allowEdit={false} onContentUpdated={onContentUpdated} onElementDeleted={onElementDeleted}>
      <div className="space-y-3">
        {editing ? (
          <InlineEditorShell title="List Paragraph" isDirty={isDirty} status={status} error={error} onSave={handleSave} onCancel={handleCancel}>
