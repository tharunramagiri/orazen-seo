...
function TableOfContents({ elements }: { elements: { id: string; element_type: string; content: Record<string, unknown> }[] }) {
  const headings = elements
    ...
    .map((el) => ({
      id: el.id,
      title: el.content.title as string,
      isIntro: el.element_type === 'introduction',
      isConclusion: el.element_type === 'conclusion',
    }))
      ...
      <ul className="space-y-1.5 border-l border-neutral-200 pl-4">
        {headings.map((h) => (
          <li key={h.id}>
            <a
              href={`#${h.id}`}
              className="block text-[13px] text-neutral-500 hover:text-blue-600 transition-colors leading-snug py-0.5"
            >
              {h.title}
            </a>
          </li>
  ...
  return (
    <div className="mx-auto max-w-[1120px] px-6 py-12">
      <nav className="mb-6 text-[13px] text-neutral-400">
        ...
        <Link href="/example/blog" className="hover:text-blue-600 transition-colors">Blog</Link>
        <span className="mx-1.5">/</span>
      ...
      <div className="mt-12 grid gap-4 border-t border-neutral-200 pt-8 md:grid-cols-2">
        {prevPost ? (
          <Link
            href={`/example/blog/${prevPost.slug}`}
            className="group flex items-center gap-3 rounded-lg border border-neutral-200 p-4 hover:border-blue-200 transition-colors"
          >
        ...
        {nextPost ? (
          <Link
            href={`/example/blog/${nextPost.slug}`}
            className="group flex items-center justify-end gap-3 rounded-lg border border-neutral-200 p-4 hover:border-blue-200 transition-colors text-right"
          >
            <div className="min-w-0">
          ...
          {relatedPosts.map((related) => (
            ...
            >
              ...
              <div className="p-4">
                ...
                <p className="text-[14px] font-semibold text-neutral-900 group-hover:text-blue-600 transition-colors leading-snug line-clamp-2">
                  {related.title}
                </p>
                <p className="mt-1.5 text-[12px] text-neutral-500 line-clamp-2">{related.excerpt}</p>
              </div>
            </Link>
          ))}
        </div>
